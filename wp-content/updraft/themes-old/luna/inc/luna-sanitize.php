 ...
 * @since 1.7
 */
function luna_sanitize_index_content( $content ) {
	if ( 'option2' == $content ) {
		return 'option2';
	} else {
		return 'option1';
...
//Integers
function luna_sanitize_int( $input ) {
    if( is_numeric( $input ) ) {
        return intval( $input );
    }
}
...
//Menu
function luna_sanitize_menu_toggle_display( $input ) {
    $valid = array(
      'icon' => '',
      'label' => '',
      'icon-label' => ''
...
function luna_sanitize_widget_content( $input ) {
    $valid = array( 
        'option1' => '1 Column',
		'option2' => '2 Columns',
		'option3' => '3 Columns',  
...
//Sanitizes Fonts 
function luna_sanitize_fonts( $input ) {  
    $valid = array(
			' ',
...
function luna_no_sanitize( $input ) {
} 
