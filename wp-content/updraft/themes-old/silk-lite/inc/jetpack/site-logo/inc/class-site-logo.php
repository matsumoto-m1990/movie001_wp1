...
//		}
	...
	public function preview_enqueue() {
		wp_enqueue_script( 'site-logo-preview', get_template_directory_uri() . '/inc/jetpack/site-logo/js/site-logo.js', array( 'media-views' ), '', true );
		...
		if ( ! current_theme_supports( 'custom-header' ) ) {
			...
			wp_enqueue_script( 'site-logo-header-text', get_template_directory_uri() . '/inc/jetpack/site-logo/js/site-logo-header-text.js', array( 'media-views' ), '', true );
	...
	public function add_media_state( $media_states ) {
		...
		if ( $this->has_site_logo() ) {
			...
			if ( $post->ID == $this->logo['id'] ) {
				$media_states[] = esc_html__( 'Site Logo', 'jetpack' );
			}
		}
